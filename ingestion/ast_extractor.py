...
Walks a Tree-sitter parse tree and extracts UPG nodes.
Each language has its own visitor method that knows the
AST node-type names for that grammar, but ALL visitors
produce the same UPGNode types — that's the language-agnostic contract.
"""
...
class ASTExtractor:
    """
    Given a source file, parse it and populate a UPGGraph.

    Usage:
        extractor = ASTExtractor(repo_root="/path/to/repo")
        graph = UPGGraph(repo_root="/path/to/repo")
        extractor.extract_file(Path("src/service.py"), graph)
    """
    ...
    def extract_file(self, abs_path: Path, graph: UPGGraph) -> int:
        """
        Parse one file and add its nodes to graph.
        ...
        visitor(tree.root_node, source_str, rel_path, language, graph)
    ...
    @staticmethod
    def _text(node: TSNode, source: str) -> str:
    ...
    @staticmethod
    def _child_text(node: TSNode, field: str, source: str) -> str:
        child = node.child_by_field_name(field)
        return source[child.start_byte:child.end_byte] if child else ""

    def _param_names(self, params_node: TSNode, source: str) -> list[str]:
        ...
        for child in params_node.children:
            if child.type in ("identifier", "typed_parameter",
                ...
                # Just grab the first identifier token
    ...
    def _collect_calls(self, node: TSNode, source: str) -> list[str]:
        """Recursively collect all called function names under a node."""
        calls = []
        def walk(n: TSNode):
            if n.type == "call":
    ...
    def _visit_python(self, root: TSNode, source: str,
                      file_path: str, lang: str, graph: UPGGraph) -> None:
        def walk(node: TSNode, current_class: str = ""):
            if node.type == "function_definition":
                ...
                # Walk inside the function (nested functions / methods)
    ...
    def _visit_javascript(self, root: TSNode, source: str,
                          file_path: str, lang: str, graph: UPGGraph) -> None:
        def walk(node: TSNode, current_class: str = ""):
    ...
    def _visit_typescript(self, root: TSNode, source: str,
                          file_path: str, lang: str, graph: UPGGraph) -> None:
        # TypeScript grammar is a superset of JS grammar in tree-sitter
        self._visit_javascript(root, source, file_path, lang, graph)

    # ── Java Visitor ──────────────────────────────────────────────────────────

    def _visit_java(self, root: TSNode, source: str,
                    file_path: str, lang: str, graph: UPGGraph) -> None:
        def walk(node: TSNode, current_class: str = ""):
    ...
    def _visit_cpp(self, root: TSNode, source: str,
                   file_path: str, lang: str, graph: UPGGraph) -> None:
        def walk(node: TSNode, current_class: str = ""):
            if node.type == "function_definition":
    ...
    def _visit_go(self, root: TSNode, source: str,
                  file_path: str, lang: str, graph: UPGGraph) -> None:
    ...
    # ── Rust + C# + Generic fallback ─────────────────────────────────────────

    def _visit_rust(self, root: TSNode, source: str,
                    file_path: str, lang: str, graph: UPGGraph) -> None:
        self._visit_generic(root, source, file_path, lang, graph)

    def _visit_csharp(self, root: TSNode, source: str,
                      file_path: str, lang: str, graph: UPGGraph) -> None:
        self._visit_generic(root, source, file_path, lang, graph)

    def _visit_generic(self, root: TSNode, source: str,
                       file_path: str, lang: str, graph: UPGGraph) -> None:
        """
        Fallback visitor: extracts anything that looks like a named function
        or class using common node type names shared across many grammars.
        """
        FUNC_TYPES = {
